...
2. To upload all the data with a gremlin query, on **Data Explorer**, expand **blueyonder**, expand **traveler**, and then click **Graph**.
3. Click **folder**, in [Repository Root]**\Allfiles\Mod07\LabFiles\Lab2\Assets**, copt the content of the **GraphData.txt** file, and then click paste it to the text box, and then click **Execute Gremlin Query**.
4. To run the Gremlin query to return all the vertices, delete the text in the text box, and type **g.V()**, and then click **Execute Gremlin Query**.
